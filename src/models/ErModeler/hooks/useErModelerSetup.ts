...
export const useErModelerSetup = (
  ...
  useEffect(() => {
    if (!canvasRef.current) {
      logger.warn(
        "Container ER DOM não está disponível, aguardando...",
        "ER_SETUP"
      );
      ...
      await safeAsyncOperation(
        async () => {
          ...
          try {
            ...
            logger.debug(
              "Modeler ER e canvas prontos para importação",
              "ER_SETUP"
            );
          } catch (canvasError) {
        ...
        },
        {
          ...
          fallback: () => {
            logger.warn(
              "ER Modeler não pôde ser inicializado completamente",
              "ER_SETUP"
            );
            setError(
              "Erro na inicialização. Algumas funcionalidades podem estar limitadas."
            );
            setLoading(false);
              ...
              try {
                if (!modelerRef.current && canvasRef.current) {
                  ...
                  notifications.warning(
                    "ER Editor em modo básico - algumas funcionalidades limitadas"
                  );
                  logger.info("ER Modeler fallback ativo", "ER_SETUP");
                }
              } catch (fallbackError) {
                logger.error(
                  "Fallback do ER Modeler também falhou",
                  "ER_SETUP",
    ...
    const setupEventListeners = () => {
      ...
      try {
        ...
        eventBus.on("selection.changed", (event: any) => {
          ...
          logger.debug(
            `Seleção alterada: ${newSelection.length} elementos`,
            "ER_INTERACTION"
          );
        });
        ...
        logger.error(
          "Erro ao configurar event listeners ER",
          "ER_SETUP",
          eventError as Error
        );
      }
      ...
      setTimeout(() => {
        safeOperation(
          ...
          },
          {
            ...
            userMessage:
              "Erro no controle do minimap ER. Funcionalidade pode estar limitada.",
            showNotification: false,
            fallback: () => {
  ...
  const importDiagram = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    logger.info(
      `Iniciando importação do arquivo ER: ${file.name}`,
      "ER_IMPORT"
    );
      ...
      await safeAsyncOperation(
        async () => {
          ...
          notifications.success(
            `Diagrama ER "${file.name}" importado com sucesso!`
          );
          logger.info(
            `Diagrama ER importado com sucesso: ${file.name}`,
            "ER_IMPORT"
          );
        ...
        },
        {
          ...
          fallback: () => {
            logger.warn(
              `Fallback: Limpando input de arquivo ER após falha na importação`,
              "ER_IMPORT"
            );
            event.target.value = "";
  ...
  const handleFitAll = () => {
    ...
    safeOperation(
      () => {
        ...
        logger.info(
          "Fit All ER executado - canvas ajustado para mostrar todos os elementos",
          "ER_CANVAS_OPERATION"
        );
        notifications.info(
