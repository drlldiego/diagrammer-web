...
export const useErUnsavedChanges = () => {
  ...
  useEffect(() => {
    const handleBeforeUnload = (e: BeforeUnloadEvent) => {
      if (hasUnsavedChanges && !showExitModal) {
        e.preventDefault();
        logger.info('Usuário tentou sair com mudanças ER não salvas', 'ER_UNSAVED_CHANGES');
  ...
  useEffect(() => {
    const handlePopState = (e: PopStateEvent) => {
      if (hasUnsavedChanges && !showExitModal) {
        ...
        logger.info('Navegação bloqueada devido a mudanças ER não salvas', 'ER_UNSAVED_CHANGES');
      }
    };
  ...
  const handleImportDone = () => {
    logger.debug('Import ER concluído - resetando estado de mudanças', 'ER_UNSAVED_CHANGES');
    setHasUnsavedChanges(false);
  };
