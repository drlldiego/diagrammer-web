...
export class ErDiagramSerializer {
  ...
  canSerialize(): boolean {
    ...
    const erElements = allElements.filter(
      (element: any) =>
  ...
  private extractErDiagramFromCanvas(): ErDiagram {
    ...
    // Segundo passo: identificar atributos
    for (const element of allElements) {
    ...
    // Terceiro passo: identificar conexões e relacionamentos
    for (const element of allElements) {
  ...
  private isEntity(element: any): boolean {
    return (
      element.businessObject?.erType === "Entity" ||
  ...
  private isAttribute(element: any): boolean {
    return (
      element.businessObject?.erType === "Attribute" ||
  ...
  private extractRelationship(
    ...
    const cardinalitySource =
      this.getStringProperty(businessObject, "cardinalitySource") || "1..1";
    const cardinalityTarget =
      this.getStringProperty(businessObject, "cardinalityTarget") || "1..1";

    // Converter cardinalidades para formato Mermaid
    const mermaidCardinality = this.convertCardinalitiesToMermaid(
      cardinalitySource,
      cardinalityTarget
    );

    const isIdentifying = this.getBooleanProperty(
      businessObject,
      "isParentChild"
    );
  ...
  private convertCardinalitiesToMermaid(
    sourceCard: string,
    targetCard: string
  ): string {
    ...
    const normalizeCardinality = (
      ...
      // One-or-Many (pelo menos um)
      if (
        normalized === "1..*" ||
        normalized === "1..n" ||
        normalized === "one-or-many" ||
      ...
      // Zero-or-Many (nenhum ou muitos)
      if (
        normalized === "0..*" ||
        normalized === "0..n" ||
        normalized === "zero-or-many" ||
        normalized === "many" ||
        normalized === "*"
      ) {
        return "zero-or-many";
