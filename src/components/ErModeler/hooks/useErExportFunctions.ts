...
export const useErExportFunctions = (modelerRef: React.MutableRefObject<BpmnModeler | null>) => {
  ...
  const exportDiagram = async () => {
    ...
    await safeAsyncOperation(
      ...
      },
      {
        ...
        fallback: () => {
          logger.warn('Fallback: Tentando salvar ER XML em formato não formatado', 'ER_EXPORT');
          ...
          modelerRef.current?.saveXML({ format: false }).then(({ xml }) => {
            if (xml) {
              ...
              document.body.removeChild(a);
              URL.revokeObjectURL(url);
              notifications.warning('Diagrama ER exportado em formato básico devido a erro');
            }
          }).catch(() => {
    ...
    await safeAsyncOperation(
      async () => {
        ...
        return new Promise<void>((resolve, reject) => {
          img.onload = function () {
            logger.debug(`Dimensões SVG ER originais: ${img.width}x${img.height}`, 'ER_PDF_EXPORT');
            ...
            logger.debug(`PDF ER: ${mmWidth.toFixed(1)}x${mmHeight.toFixed(1)}mm`, 'ER_PDF_EXPORT');
            pdf.addImage(imgData, "PNG", 0, 0, mmWidth, mmHeight, undefined, 'SLOW');
            
            logger.info('PDF ER ALTA QUALIDADE gerado com sucesso', 'ER_PDF_EXPORT');
            pdf.save("er-diagram.pdf");
            notifications.success('PDF ER de alta qualidade exportado com sucesso!');
          ...
          img.onerror = function() {
            logger.error('Erro ao carregar SVG como imagem para PDF ER', 'ER_PDF_EXPORT');
            URL.revokeObjectURL(url);
      ...
      },
      {
        ...
        userMessage: 'Erro ao exportar PDF ER. Tente usar outro formato ou verifique se o diagrama está válido.',
        fallback: () => {
          logger.warn('Fallback: Tentando exportar PDF ER com qualidade reduzida', 'ER_PDF_EXPORT');
  ...
  const exportToPNG = async () => {
    ...
    logger.info('Iniciando exportação PNG ER com qualidade máxima', 'ER_PNG_EXPORT');
    ...
    await safeAsyncOperation(
      async () => {
        ...
        return new Promise<void>((resolve, reject) => {
          img.onload = function () {
            logger.debug(`Dimensões SVG ER originais: ${img.width}x${img.height}`, 'ER_PNG_EXPORT');
          ...
          img.onerror = function() {
            logger.error('Erro ao carregar SVG como imagem para PNG ER', 'ER_PNG_EXPORT');
            URL.revokeObjectURL(url);
      ...
      },
      {
        ...
        userMessage: 'Erro ao exportar PNG ER. Tente novamente ou use outro formato.',
        fallback: () => {
          logger.warn('Fallback: Tentando exportar PNG ER com qualidade reduzida', 'ER_PNG_EXPORT');
          // Fallback: tentar exportar como SVG
          modelerRef.current?.saveSVG().then(({ svg }) => {
            const link = document.createElement('a');
            const blob = new Blob([svg], { type: 'image/svg+xml' });
            ...
            link.download = 'er-diagram-fallback.svg';
            link.click();
            notifications.warning('Exportado como SVG devido a erro no PNG ER');
          }).catch(() => {
            notifications.error('Não foi possível exportar nem em PNG nem em SVG');
          });
        }
      }
    );
