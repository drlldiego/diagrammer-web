...
(ErElementFactory as any).prototype.createShape = function(this: any, attrs: ErAttributes): Element {
  ...
  console.log('🏭 ErElementFactory: createShape chamado com attrs:', attrs);
  console.log('🏭 ErElementFactory: tipo BPMN:', attrs.type, 'erType:', erType);
  
  // Criar elemento base usando o factory padrão
  const element = this._elementFactory.createShape(otherAttrs);
  
  console.log('🏭 ErElementFactory: elemento criado:', element.id, 'tipo:', element.type);
  ...
  if (erType && element.businessObject) {
    ...
    console.log('✅ ErElementFactory: Propriedades ER definidas para', element.id, 'erType:', erType, '$attrs:', element.businessObject.$attrs);
  }
